...
echo "$clave" >> ~/.ssh/authorized_keys                  #directo a MINIPC 

ssh-keyscan -H 192.168.1.46 >> ~/.ssh/known_hosts        #desde MINIPC a debian#
sshpass -p "pi3" ssh -t pi3@192.168.1.46 << EOF
echo "$clave" >> ~/.ssh/authorized_keys
EOF

ssh-keyscan -H 192.168.1.35 >> ~/.ssh/known_hosts        #desde MINIPC a juan#
sshpass -p "lynx.2023" ssh -t juan.gonzalez@192.168.1.35 << EOF
echo "$clave" >> ~/.ssh/authorized_keys
EOF


echo "$(date '+%c') || SSH_containers || Node-Red hecho ||" >> LogsJM/updates.txt


############
echo "CAMBIO DOCKER HOME ASSISTANT"
############
...
echo "$(date '+%c') || SSH_containers || Homeassistant hecho ||" >> LogsJM/updates.txt

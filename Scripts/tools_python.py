...
def log_updates_containers(programa,mensaje):

    log = "date +'%c || " + programa + " || " + mensaje + " ||'  >> LogsJM/updates_containers.txt "
...
def obtener_nombre_serie(line):
    ...
    nombre_TMDB = re.split(r'(\s[0-9]{4}\s)', nombre_TMDB)[0]          # filtramos por año (xxxx)
    ...
    nombre_TMDB = re.split(r'Temp', nombre_TMDB)[0]
...
def rename(line,title,release_date,formato,height,duration,monitor,titulo_episodio="N/A",season="N/A",episode="N/A"):
    ...
    print("rename:")
    print("titulo ep " + titulo_episodio)
    print("season " + season)
...
def directorio_exp_2(nombre_directorio):
    ...
    directorio_nuevo = f"Expansion_2/Varios_Monitor/{nombre_directorio}"     
...
def makemkvcon(directorio_nuevo,line):
    ...
    command = f'mkdir "{directorio_nuevo}"'   ## comillas para los espacios
    subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

    makemkv = f'makemkvcon --debug  mkv \"{line}\" all "{directorio_nuevo}"'
    print(makemkv)    
    subprocess.run(makemkv, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)


def makemkvtitles(line):

    makemkv = f'makemkvcon info \"{line}\" | grep Total'
    result = subprocess.run(makemkv, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    result = result.stdout
    cadena_sin_espacios = result.rstrip()
    log_monitor("makemkvISO",cadena_sin_espacios)
